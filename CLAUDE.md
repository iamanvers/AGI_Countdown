...
`methodology.json`. The first 10 are validated by `packages/validate` (`pnpm validate:data`);
`methodology.json` is derived from the registries (not separately validated).
